...
int Frontend::select_attrlist_from_table_where(char relname_source[ATTR_SIZE], char relname_target[ATTR_SIZE],
                                               ...
                                               char attribute[ATTR_SIZE], int op, char value[ATTR_SIZE])
{
  char tarRelTemp[ATTR_SIZE] = TEMP;
  int ret = Algebra::select(relname_source, tarRelTemp, attribute, op, value);
  if (ret != SUCCESS)
  {
    return ret;
  }

  int tempRelId = OpenRelTable::openRel(tarRelTemp);
  if (tempRelId < 0 || tempRelId >= MAX_OPEN)
  {
    Schema::deleteRel(tarRelTemp);
    return tempRelId;
  }

  ret = Algebra::project(tarRelTemp, relname_target, attr_count, attr_list);
